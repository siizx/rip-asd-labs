...
{
try {
    string nome_file, nome_file_res, nome_file_qsb, nome_file_qsr;
    ...
    cin >> nome_file;
    if (!cin) throw runtime_error("Errore inserimento nome file\n"); 
    nome_file_res = "ok-"+nome_file;
    nome_file_qsb = "qsb-"+nome_file;
    nome_file_qsr = "qsr-"+nome_file;
    ...
    ofstream ofs2(nome_file_res.c_str()); // apertura di uno stream associato ad un file, in scrittura 
    stampaVettoreEDurataSuStream(v1, "", 0, ofs2); 
    ...
    ofstream ofs3(nome_file_qsb.c_str()); // apertura di uno stream associato ad un file, in scrittura 
    stampaVettoreEDurataSuStream(v5, "", 0, ofs3); 
    ...
    ofstream ofs4(nome_file_qsr.c_str()); // apertura di uno stream associato ad un file, in scrittura 
    stampaVettoreEDurataSuStream(v6, "", 0, ofs4); 

    cout << "Verificate con diff "<< nome_file_res << " " << nome_file_qsb << "\n";
    cout << "e diff "<< nome_file_res << " " << nome_file_qsr << "\n\n";
